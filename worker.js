...
async function handleMessage(e) {
    ...
    try {
        ...
        if (action === "addLanbookPage") {
            if (!doc) throw new Error("PDF документ не инициализирован");
            const { imageData, width, height } = e.data;

            // Add a new page matching the canvas dimensions
            doc.addPage({ size: [width, height] });

            // Insert the image into the PDF. x=0, y=0, with dimensions of the canvas
            doc.image(imageData, 0, 0, { width: width, height: height });
